---
文档性质: 技术简报
最后修订日期: { YYYY-MM-DD }
作者: { Agent名称 - Agent平台名称}
发起人: { 用户姓名 }
受众: { 读者画像 —— 例如"物理化学方向研究生（熟悉电子跃迁 / 分子光谱基础；不熟悉原子核物理）"；用户主动说"我不懂"时必须明确写出"不熟悉 XXX" }
---

# {技术名称}技术简报

## 目录

1.  [一句话定义](#1-一句话定义)
2.  [术语 / 名词表](#2-术语--名词表)
3.  [核心技术原理](#3-核心技术原理)
4.  [原理示意图](#4-原理示意图)
5.  [基本公式方程](#5-基本公式方程)
6.  [技术变体 / 发展](#6-技术变体--发展可选)
7.  [数据处理算法](#7-数据处理算法可选)
8.  [典型示例应用](#8-典型示例应用)
9.  [注意事项与工程要点](#9-注意事项与工程要点)
10. [参考文献](#10-参考文献)
11. [依赖地图](#11-依赖地图)

---

## 1. 一句话定义

> **{技术名}** = {用一句话说清楚是什么、做什么}
>
> **英文名**：{English name}（{aliases}）
>
> **典型应用场景**：{一两个最常见的应用领域，1-2 句话}

---

## 2. 术语 / 名词表

### 2.1 专有名词 + 公式物理量符号

**专有名词（名称 / 缩写）**：

| 术语 | 含义 | 首次正式定义章节 |
| :-: | :-- | :-- |
| **{专有名词 A}** | {一句话释义} | {§X.X} |
| **{专有名词 B}** | ... | ... |

**公式物理量符号**（{技术名} 涉及的核心符号类物理量）：

| 符号 | 含义 | 数值 / 常用取值 | 首次正式定义章节 |
| :-: | :-- | :-- | :-- |
| $X$ | {含义} | {典型数值 / 单位} | {§X.X} |
| $Y_r$ | ... | ... | ... |

### 2.2 同义术语与别名

| 术语 / 名词 | 别名 |
| :-: | :-- |
| **{唯一用名 A}** | 别名 1 / 别名 2 / 缩写 |
| **{唯一用名 B}** | ... | ... |

### 2.3 意思相近术语 + 分类标准歧义术语

| 术语对 / 单术语 | 区分说明 / 分类标准 |
| :-: | :-- |
| **{意思相近术语 A} vs {B}** | {同源但有明确区别，无法彼此替代——区分说明} |
| **{意思相近术语 C} vs {D}** | ... |
| **{分类标准歧义术语 E}** | 按 {标准 A} 分类：{归类}；按 {标准 B} 分类：{归类} |
| **{其它需显式区分说明的术语 F}** | {区分说明} |

---

## 3. 核心技术原理

### 3.1 技术背景

> {前置概念}

#### {Happy path}

#### {Fail / 反直觉}

#### {Fix / 解法}

### 3.2 ★ 灵魂锚点

| 关键要素 1 | 关键要素 2 | 关键推论 |
| :-: | :-: | :-: |
| ... | ... | ... |

### 3.3 {子原理 A}

{分层 / 分链拆解；可含多张对比表}

### 3.4 {子原理 B}

{……}

---

## 4. 原理示意图

### 4.1 流程图（Mermaid，默认 flowchart TD）

```mermaid
flowchart TD
    A["{步骤 A}"] --> B["{步骤 B}"]
    B --> C["{结果}"]
```

### 4.2 几何 / 谱图示意（外部 SVG）

![{几何/谱图示意}](assets/{geometric.svg})

### 4.3 配套交互演示（可选，但推荐）

🔗 直接打开：`交互演示.html`（浏览器双击即可，零依赖）

---

## 5. 基本公式方程

### 5.1 {公式名称}

$$
\boxed{\text{公式}}
$$

- **符号说明**：
  - $x$：含义（量纲）
  - $y$：含义（量纲）

- **数值例子**：代入 {典型参数} → 结果 = {具体数字}

- **适用条件**：{公式成立的前提 / 边界}（如：瑞利区适用 / Mie 区仅作机理理解）

### 5.2 {公式名称 2}

{……}

---

## 6. 技术变体 / 发展（可选）

| 变体类型 | 核心差异 | 适用场景 | 与主体的关系 |
| :-: | :-- | :-- | :-- |
| {变体 A} | ... | ... | ... |
| {变体 B} | ... | ... | ... |

---

## 7. 数据处理算法（可选）

| 算法 | 原理 | 优势 | 适用场景 |
| :-: | :-- | :-- | :-- |
| {……} | {……} | {……} | {……} |

---

## 8. 典型示例应用

### 8.1 【本简报触发文献】{用途简述}

- **文献**：{作者}. {标题}. {期刊} **{年份}**, **{卷(期)}**, {页}.
- **链接 / DOI**：[{URL}]({URL})（DOI: `{DOI}`，开放获取 / 受限）

**用到本技术 / 变体的具体位置**：
1. {在本技术体系中，这篇论文用到了哪部分——精确到段落 / 章节 / 图号级别}
2. {……}
3. {……}

- **意义 / 可借鉴点**：{一句话总结为何值得引用，以及其它研究者如何借鉴}

### 8.2 {其它典型应用（可选）}

| 应用 | 该技术发挥的作用 |
|:---|:---|
| ... | ... |

---

## 9. 注意事项与工程要点

### 9.1 缺点 / 弱项 / 不足 / 局限束缚

- {缺点 1}
- {缺点 2}
- {缺点 3}

### 9.2 踩坑框

| # | 要点 | 说明 |
|:---:|:---|:---|
| 1 | {踩坑 1} | {踩坑原因 + 规避方式} |
| 2 | {踩坑 2} | {……} |
| 3 | {踩坑 3} | {……} |

> ⚠️ **常见反模式**：
> - ❌ {反模式 1}
> - ❌ {反模式 2}

---

## 10. 参考文献

**经典文献（按时间正序）**：

1. {Author A; Author B. *Title.* Journal Year, Vol(Issue), Pages.} DOI: {DOI}
2. {……}

**综述**：

3. {Author X. *Review title.* Journal Year.} DOI: {DOI}
4. {……}

**应用论文（按依赖强度排序）**：

5. {User-provided paper for this brief.} DOI: {DOI}

---

## 11. 依赖地图

| 依赖强度 | 技术 | 在 A 中的角色 | 是否需要独立简报 |
|:---|:---|:---|:---|
| 🔴 强依赖 | {拉曼光谱基础 等} | {信号产生机理 等} | {⚠️ 待询问 / 否} |
| 🟡 弱依赖 | {LSPR 等} | {增强机理引用 等} | {否} |
| ⚪ 仅提及 | {电化学工作站 等} | {工具引用 等} | {否} |

> {补充备注}
