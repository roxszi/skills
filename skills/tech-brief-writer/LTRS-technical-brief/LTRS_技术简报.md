...
5. [原理示意图](#5-原理示意图)
6. [基本公式方程](#6-基本公式方程)
7. [技术变体 / 发展：结构光光阱拉曼](#7-技术变体--发展结构光光阱拉曼)
8. [数据处理算法](#8-数据处理算法)
9. [典型示例应用](#9-典型示例应用)
...
>
> 英文别名：Raman Tweezers / Raman Tweezers Spectroscopy (RTS)。

---

## 4. 核心技术原理

### 4.1 ★ 单一核心洞察：单光束内"双作用合一"
...
>
> **结构化光场**（见 §7）的本质：**把"光束→粒子"和"光束→散射信号"这两条因果链在同一焦面上解耦开**——一份光场负责偏左偏右捕，一份单独配对 Raman 激发，做不做得到"分区探测"，决定了能不能"看膜 vs 看胞质"。

### 4.2 光镊捕获（Optical Trapping）
...
2. **多光束式**：一束（或两束）负责捕获/拉伸细胞，另一束独立负责拉曼激发。可解耦"捕获功率"与"激发功率"，并支持细胞拉伸等力学操作（见 §9）。

---

## 5. 原理示意图

> 📌 **硬性要求**：Mermaid 讲流程/因果，外部 SVG（`assets/*.svg`）讲几何/谱线形态，二者互补。所有 SVG 一律外置，md 中只引用不内嵌（详见 §三·3.3）。

### 5.1 经典 LTRS 光路

![LTRS 经典光路示意图：激光器 → 扩束 → 二向色镜 → 高 NA 物镜 → 紧聚焦到细胞 → 背向拉曼散射 → Notch 滤波 → 光谱仪 + CCD。焦点处标注梯度力 F_grad（蓝）与散射力 F_scat（红）。](assets/optical_path.svg)

> 📁 **文件**：`assets/optical_path.svg`（外置 SVG）
> - **跨平台渲染稳健**：所有元件 / 箭头均以绝对像素坐标 + `text-anchor` 定位，不依赖任何字符宽度
> - **跨字体一致**：`font-family="system-ui, ..."` 兜底，不用等宽字体强行对齐
> - **可被 HTML demo 复用**（demo 也可直接 `<img src="assets/optical_path.svg">` 引用，**单一来源**）

### 5.2 流程图（Mermaid：信号链路与控制回路）
...
### 5.3 三种光阱的焦面形态（Point / Donut / Line）

![LTRS 三种结构光场焦面形态对比：左侧高斯基模点聚焦（仅探胞质）；中间涡旋光束环形光阱带暗核（专探膜）；右侧光片线聚焦（最全包含，酰胺 III 带尤丰富）。](assets/trap_geometries.svg)

> 📁 **文件**：`assets/trap_geometries.svg`（外置 SVG）
> - **配色语义化**：Point = 蓝（探胞质），Donut = 绿（专探膜），Line = 红（全包含）
> - **暗核用 mask 真实实现**（而非两个圆相减的字符级近似），跨字体一致
> - **可被 HTML demo 第 (C) 面板复用为静态背景层**

### 5.4 配套交互演示

> **`ltrs_core_principle.html`**（与本文档同目录，双击离线打开，零依赖）
> 包含 4 个面板的交互演示：
...
> - **(D) λ ↔ 拉曼信号 ↔ 光损伤**：直观展示 §4.1 中"三股力 trade-off"在工作窗上的体现
>
> **可调参数**：功率、NA、粒子半径 a、波长 λ、几何形貌（5 个滑块）
>
> **与本简报的 SVG 资源耦合**：
> - `ltrs_core_principle.html` 的 (C) 面板中三种光场形态的代码逻辑，可与外置 SVG `assets/trap_geometries.svg` 共享参数 → 后续优化时可让 demo 直接 `<img src="assets/trap_geometries.svg">` 引用静态版本（按 skill §三·3.3 第 4 条规范"单一来源，单份维护"）
...
> **算例 + 适用条件**：从 1064 nm 改到 785 nm 激发，频率提升 1.35 倍 ⇒ 拉曼散射截面 ∝ `ν⁴` 提升约 **3.3 倍**。这是为何教学/科研常用可见–近红外（532/633/785 nm）而非深红 1064 nm：信噪比改善，但要付出"荧光背景与光损伤加剧"的代价——这又是 §4.1 中 trade-off 的具体体现。
>
> **适用条件**：ν⁴ 律在散射截面远小于入射时成立（绝大多数实验条件满足）；近共振 / 共振拉曼条件下需做共振修正。

---

## 7. 技术变体 / 发展：结构光光阱拉曼（Structured Optical Trap Raman）

### 7.1 经典 LTRS 的固有短板

经典 LTRS 用**高斯基模（TEM₀₀）点聚焦**，激发斑极小，只探到细胞**内部（主要是胞质 cytoplasm）**，**漏掉细胞膜**这一关键组分。对红细胞（RBC）而言，膜的力学/生化信息恰恰是病理诊断的核心。

### 7.2 解决思路：把激发/捕获光场"结构化"

用**结构光（structured light）**替换高斯点，改变焦点处光强的空间分布，从而**主动选择探测细胞的哪一部分**：

| 光阱类型 | 光场 | 焦斑形状 | 对细胞膜的探测特性 |
|---|---|---|---|
| **点聚焦 Point** | 高斯基模 TEM₀₀ | 极小点（见 `assets/trap_geometries.svg` A 面板） | 几乎只探胞质，**排斥膜**（经典 LTRS） |
...
> 这正是 §9 中 **Sarmah et al. (2026, Anal. Chem.)** 的核心方法学：在同一 LTRS 框架下并列比较 point / donut / line 三种光阱对 RBC 拉曼谱的贡献差异。
...
| 6 | **瑞利 vs Mie 公式** | §6 的瑞利式严格适用于 `a ≪ λ`；活细胞（μm 级）属 Mie 区，**不能直接用瑞利公式做定量力估算** |
...
- 定量力学分析中，μm 级细胞属 Mie 区，瑞利公式仅作机理理解（见 §6 警示）。

---

## 12. 参考文献（末尾必填）

> 📌 本节按 SKILL.md §一·12 末尾必填。**完整 DOI 核实流程与分类排列规则见 §三·3.2**。标注 `（待核实）` 者为本简报凭记忆给出、尚未在本次检索中直接核对的编号，请审核时优先复核。
...
7. Zhu, T. 等（综述）. *Optical Tweezers in Studies of Red Blood Cells.* Cells **2020**, 9(3), 545. DOI: 10.3390/cells9030545 **（作者名待核实）**
...
*本文档由 CherryClaw 与司承运在 2026-07-06 起的讨论中陆续沉淀。最初版本仅为文字+公式；后按 `skills/technical-brief-writer/SKILL.md` 多次升级：*

- *v1（2026-07-06）：文字 + 公式 + Mermaid，单核心洞察缺失；*
- *v2（2026-07-07）：补 §1 「单一核心洞察」作为整篇灵魂；§3 改 ASCII 草图为 inline SVG；新增 §6 数据处理算法表（PCA / PLS / MCR-ALS / ANOVA）；新增 §11 记忆口诀；全文加 ⚠️ 工程踩坑警示与算例；*
- *v3（2026-07-07）★：按 `SKILL.md` v2 升级——将 inline SVG **外置到 `assets/`**（`optical_path.svg` + `trap_geometries.svg`），md 中改为 `![...](assets/xxx.svg)` 引用；建立 `scripts/` 目录；本简报正式成为 `SKILL.md` 的 SVG 外置化最新范本。*
- *v4（2026-07-07）★：按 `SKILL.md` v2.2.0 升级—— **重映射章节编号为 §一 模板的 14 章节**（文档头与一句话定义独立成节；原 §1-12 章节重新编号）；**新增 §14 依赖地图**（按 SKILL §三·3.4 末尾必填）。*
- *v5（2026-07-07）★：按 `SKILL.md` v3.0.0 升级—— **新增 §12 参考文献**（按 §三·3.2 末尾必填）；原 §12 记忆口诀 / §13 溯源脚注 / §14 依赖地图依次右移到 §13 / §14 / §15；删除原"附录：参考文献"段（已被 §12 吸收）。*
...
> 📌 本节按 SKILL.md §三·3.4 末尾必填（对应 §一·15）。**LTRS = 复合技术**，主体是"光镊 + 拉曼联用"，对其他技术有强依赖（须识别哪些 B 需独立简报）。
...
| 🟡 弱依赖 | **化学计量学算法包**（PCA / PLS / MCR-ALS / ANOVA） | 数据处理常规工具（§8） | 否 |
| ⚪ 仅提及 | 高 NA 显微物镜 / Notch 滤波片 / EMCCD | 工具引用 | 否 |
| ⚪ 仅提及 | 结构光光场生成（涡旋光束 / 光片） | §7 变体的实现工具 | 否 |

> **配套询问句式**（CherryClaw 给司承运的询问——禁止默认展开）：
>
> > 本简报在 **§6 基本公式方程** 和 **§7 结构光变体** 处强依赖 **拉曼光谱基础** 与 **光镊基础** 两项基础技术，但按 §三·3.4 规则未在本文展开。
> >
> > 是否需要我（CherryClaw）单独撰写这两份技术简报？
> > （如需，请确认侧重方向：基础原理 / 工程实践 / 文献综述 / ICH-style 方法学）
> >
> > ⚠️ 注：若按"基础原理"方向撰写，预计每份 ~150-300 行；链式触发的下一层依赖（如"分子振动基础"在拉曼简报里）将在 B 简报里再展开。
