...
| [`tech-brief-writer`](#技术简报撰写) | 4.0.1 | "技术简报撰写" | — |
<!-- END: SKILLS-TABLE -->
...
1. **一句话定义** → 2. **核心技术原理**（★ 含"单一核心洞察 / 灵魂锚点"） → 3. **原理示意图**（Mermaid + 外部 SVG） → 4. **基本公式方程**（★ 含算例与适用区间） → 5. 技术变体 → 6. 数据处理算法 → 7. **典型示例应用** → 8. **⚠️ 缺点与踩坑框** → 9. **参考文献**（★ DOI 必须核实） → 10. **依赖地图**（★ 防教学法漂移）。
