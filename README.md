...
| [`tech-brief-writer`](#技术简报撰写) | 3.8.0 | "技术简报撰写" | — |
<!-- END: SKILLS-TABLE -->
...
- `notes/3.1` 公式撰写四要素（公式 / 符号 / 数值 / 条件）
- `notes/3.2` 参考文献规范（DOI 核实 + 分类排列）
- `notes/3.3` SVG 文件管理（外置 assets/ + 字体兜底 + 生成脚本留档）
- `notes/3.4` **技术依赖处理规则（避免教学法漂移）**——每个简报 B 依赖 ≤ 10% 篇幅，末尾必须询问而非默认展开
...
- `LTRS-technical-brief/` —— LTRS（光热干涉）技术简报完整示例
- `SERDS-technical-brief/` —— SERDS（移频激发拉曼差分）技术简报完整示例
